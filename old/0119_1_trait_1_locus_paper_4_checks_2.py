...
for s_i, s in enumerate(s_v):
    ...
    args = [[x,x,x,x], [av_w_pq_of_cline_v, w_pq_of_av_cline_v, [w_theo for i in range(min_len)], w_pq_of_av_cline_v_modif],\
                ...
                '0119_w_s_%.1E_F_rscl_%.2f.png' % (s, rescale * rescale_factor), labels]
    
    if rescale:
        args[-3] = 'Different estimates of w, rescale factor %.2f, s = %.1E' % (rescale_factor, s)
    ...
    if savedata:
        filenamedata = '0119_output_' + computername + '_data_s_%.1e_sigma_%.1e_rscl_%.2f.txt' % (s, sigma, rescale * rescale_factor)
