...
f.write('\n')
f.write('s, sigma, w_mle, w_pq, w_theoretical \n')
f.close()
    ...
    for sigma in sigma_v:
        ...
        while not done:
            ...
            for deme in range(K):
                ...
                new_inds = gametes.reshape(N,2)
                
                #chosen_inds = [inds_in_deme[i] for i in chosen_indices]
                #offspring = [[chosen_inds[i][0], chosen_inds[i - 1][1]] for i in range(N)]
                             
                #alleles[deme] = [[inds_in_deme[chosen_indices[i]][0], \
                #        inds_in_deme[chosen_indices[i-1]][1]] for i in range(N)]
        ...
        f = open('0118_output.txt', 'a')
        f.write('%f, %f, %f, %f, %f, %f \n' % (s, sigma, w_mean, w_2_mean, w_max, w_theo))
        f.close()
